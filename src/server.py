...
async def deduplicate_by_hash() -> str:
    ...
    report = "🗑️ Deduplication Complete!\n\n"
    report += f"Removed {result.count} duplicate files\n"
    report += "Saved space by eliminating exact copies\n"
...
async def auto_extract_and_cleanup() -> str:
    ...
    if result.count > 0:
        report += "Cleaned up original archive files\n"
...
async def clear_installers() -> str:
    ...
    report = "🧹 Installer Cleanup Complete!\n\n"
    report += f"Removed {result.count} old installer files (older than 7 days)\n"
    report += "Freed up space from unused installation files\n"
